...
Vue.directive("validate", function(el, binding, vnode, oldVnode) {
  rulesHandlers[el.getAttribute("id")] &&
    oldVnode.data.domProps &&
    vnode.data.domProps.value !== oldVnode.data.domProps.value &&
    rulesHandlers[el.getAttribute("id")](vnode.data.domProps.value);
});
...
function Validator(rules, state) {
  ...
  function isValid() {
    return (
      Object.keys(state).length > 0 && Object.keys(state).filter((ruleName) => state[ruleName] === false).length < 1
    );
  }
  return {
    isValid,
